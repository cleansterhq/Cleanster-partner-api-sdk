...
`status` describes the booking lifecycle. `cleaner_status` separately maps the
...
```ruby
resp = client.bookings.get_booking_inspection(16926)
report_url = resp.data
```

`get_booking_inspection_details` is deprecated and remains an alias for
...
| `cleaner_status` | String | Cleaner operational progress from `cleanerStatus`, independent of `status` |

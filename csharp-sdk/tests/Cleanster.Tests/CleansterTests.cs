...
public class CleansterTests
{
    ...
    {
        var http = MockHttp();
        http.Setup(h => h.PostMultipartAsync("/v1/checklist/upload-image", It.IsAny<byte[]>(), "photo.jpg", It.IsAny<CancellationToken>()))
            .ReturnsAsync(MakeResponse("{}"));
        await new ChecklistsApi(http.Object).UploadChecklistImageAsync(new byte[] { 0xFF, 0xD8 }, "photo.jpg");
        http.VerifyAll();
    }
    ...
    public async Task Checklists_UploadChecklistImage_ReturnsResponse()
    {
        ...
        http.Setup(h => h.PostMultipartAsync(It.IsAny<string>(), It.IsAny<byte[]>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(MakeResponse("{}"));
        var result = await new ChecklistsApi(http.Object).UploadChecklistImageAsync(new byte[] { 0x89, 0x50 }, "image.png");

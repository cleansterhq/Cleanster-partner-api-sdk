...
func TestBookings_CreateBooking(t *testing.T) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, okResponse(map[string]interface{}{"id": 1, "status": "CLEANER_ASSIGNED", "cleanerStatus": "ON_THE_WAY", "date": "2025-06-15", "time": "10:00", "hours": 3.0, "cost": 150.0, "propertyId": 1004, "planId": 2, "roomCount": 2, "bathroomCount": 1, "extraSupplies": false, "paymentMethodId": 10}))
	})
	client, cleanup := newTestClient(t, handler)
	defer cleanup()
	...
	if resp.Data.Status != "CLEANER_ASSIGNED" {
		t.Errorf("expected status CLEANER_ASSIGNED, got %s", resp.Data.Status)
	}
	if resp.Data.CleanerStatus != "ON_THE_WAY" {
		t.Errorf("expected cleanerStatus ON_THE_WAY, got %s", resp.Data.CleanerStatus)
	}
}
...
func TestOther_ListCleaners(t *testing.T) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/cleaners" {
			t.Errorf("wrong path: %s", r.URL.Path)
		}
		writeJSON(w, okResponse([]interface{}{}))
	})
	client, cleanup := newTestClient(t, handler)
	defer cleanup()
	_, _ = client.Other.ListCleaners(context.Background(), "", "")
}

func TestOther_GetCleaner(t *testing.T) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/cleaners/789" {
			t.Errorf("wrong path: %s", r.URL.Path)
		}
		writeJSON(w, okResponse(map[string]interface{}{}))
	})
	client, cleanup := newTestClient(t, handler)
	defer cleanup()
	_, _ = client.Other.GetCleaner(context.Background(), 789)
}

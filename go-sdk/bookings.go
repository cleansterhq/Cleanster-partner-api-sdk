...
// GetBookingInspection returns the signed URL for a completed booking's
// inspection report. Opening the URL displays before/after photos, checklist
// photo evidence, and property problems.
func (s *BookingsService) GetBookingInspection(ctx context.Context, bookingID int) (APIResponse[string], error) {
	raw, err := s.http.get(ctx, fmt.Sprintf("/v1/bookings/%d/inspection", bookingID), nil)
	if err != nil {
		return APIResponse[string]{}, err
	}
	return decode[string](raw)
}

// Deprecated: use GetBookingInspection. This compatibility alias calls the
// only public inspection endpoint; /inspection/details does not exist.
func (s *BookingsService) GetBookingInspectionDetails(ctx context.Context, bookingID int) (APIResponse[string], error) {
	return s.GetBookingInspection(ctx, bookingID)
}
...
// UpdateTask updates the task quantities for a booking.
func (s *BookingsService) UpdateTask(ctx context.Context, bookingID int, req UpdateTaskRequest) (APIResponse[map[string]interface{}], error) {
	raw, err := s.http.post(ctx, fmt.Sprintf("/v1/bookings/%d/tasks", bookingID), req)
	if err != nil {
		return APIResponse[map[string]interface{}]{}, err
	}
	return decode[map[string]interface{}](raw)
...
// UpdateSqft updates the total square footage for a booking.
func (s *BookingsService) UpdateSqft(ctx context.Context, bookingID int, req UpdateSqftRequest) (APIResponse[map[string]interface{}], error) {
	raw, err := s.http.post(ctx, fmt.Sprintf("/v1/bookings/%d/sqft", bookingID), req)
	if err != nil {
		return APIResponse[map[string]interface{}]{}, err
	}
	return decode[map[string]interface{}](raw)
